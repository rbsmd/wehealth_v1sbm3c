...
<DIV class='title_line'>
  ...
  <A href="./list_by_categoryno_grid.do?category_no=${categoryVO.category_no }" class='title_link'>${categoryVO.category_name }</A>
</DIV>
          ...
          <c:when test="${size1 > 0}"> <!-- 파일이 존재하면 -->
            <c:choose> 
              <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}"> <!-- 이미지 인경우 -->
                <a href="./read.do?itemsno=${itemsno}">               
                  <IMG src="./storage/${thumb1 }" style='width: 100%; height: 150px;'>
                </a><br>
                ${item_name} <br>
                <del><fmt:formatNumber value="${item_price}" pattern="#,###" /></del>
                <span style="color: #FF0000; font-size: 1.0em;">${discount} %</span>
                <strong><fmt:formatNumber value="${total_price}" pattern="#,###" /></strong>
              </c:when>
              <c:otherwise> <!-- 이미지가 아닌 일반 파일 -->
                <DIV style='width: 100%; height: 150px; display: table; border: solid 1px #CCCCCC;'>
                  <DIV style='display: table-cell; vertical-align: middle; text-align: center;'> <!-- 수직 가운데 정렬 -->
                    <a href="./read.do?itemsno=${itemsno}">${file1}</a><br>
                  </DIV>
                </DIV>
