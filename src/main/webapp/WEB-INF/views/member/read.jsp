  ...
  <DIV class='content_body'>    
    ...
    <FORM name='frm' id='frm' method='POST' action='./update.do' 
      ...
      <fieldset class="fieldset_basic">
        <ul>
          <li class="li_none">
            <c:set var="file1saved" value="${memberVO.file1saved.toLowerCase() }" />
            <DIV style="width: 15%; float: left; margin-right: 10px;">
              <c:choose>
                <c:when test="${memberVO.thumb1.endsWith('jpg') || memberVO.thumb1.endsWith('png') || memberVO.thumb1.endsWith('gif')}">
                  <%-- /static/member/storage/ --%>
                  <IMG src="/member/storage/${file1saved }" style="width: 100%;">
                </c:when>
                <c:otherwise> <!-- 기본 이미지 출력 -->
                  <IMG src="/member/images/none.png" style="width: 100%;">
                </c:otherwise>
              </c:choose>
            </DIV>
          ...
          <li class="li_none">
            <DIV>
              <c:if test="${file1.trim().length() > 0 }">
                첨부 파일: <A href='/download?dir=/member/storage&filename=${file1saved}&downname=${file1}'>${file1}</A> (${size1})
              </c:if>
            </DIV>
          </li>
        </ul>
